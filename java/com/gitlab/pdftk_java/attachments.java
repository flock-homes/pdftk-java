...
class attachments {
  ...
  static void unpack_file(
    ...
    if (filespec_p != null && filespec_p.isDictionary()) {
      ...
      PdfName type_p = (PdfName) input_reader_p.getPdfObject(filespec_p.get(PdfName.TYPE));
      if (type_p != null
          && type_p.isName()
          && (type_p.compareTo(PdfName.FILESPEC) == 0 || type_p.compareTo(PdfName.F) == 0)) {
        PdfDictionary ef_p =
            (PdfDictionary) input_reader_p.getPdfObject(filespec_p.get(PdfName.EF));
        if (ef_p != null && ef_p.isDictionary()) {

          // UF introduced in PDF 1.7
          PdfString fn_p = (PdfString) input_reader_p.getPdfObject(filespec_p.get(PdfName.UF));
          if (fn_p == null) { // try the F key
            fn_p = (PdfString) input_reader_p.getPdfObject(filespec_p.get(PdfName.F));
          }
          ...
          if (fn_p != null && fn_p.isString()) {
            ...
            PdfStream f_p = (PdfStream) input_reader_p.getPdfObject(ef_p.get(PdfName.F));
